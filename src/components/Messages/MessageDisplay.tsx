...
export const BaseMessageDisplay: React.FC<{ message: string }> = ({ message }) => {
  return <ReactMarkdown
    ...
    components={{
      ...
      hr({ ...props }) {
        return <hr className="py-2" {...props} />
      },
      ...
      h1({ children, ...props }) {
        return <h1 className="text-2xl font-bold py-2" {...props}>{children}</h1>
      },
      h2({ children, ...props }) {
        return <h2 className="text-xl font-bold py-2" {...props}>{children}</h2>
      },
      h3({ children, ...props }) {
        return <h3 className="text-lg font-bold py-2" {...props}>{children}</h3>
      },
      h4({ children, ...props }) {
        return <h4 className="text-base font-bold py-2" {...props}>{children}</h4>
      },
      h5({ children, ...props }) {
        return <h5 className="text-sm font-bold py-2" {...props}>{children}</h5>
      },
      h6({ children, ...props }) {
        return <h6 className="text-xs font-bold py-2" {...props}>{children}</h6>
      },
      ul({ children, ...props }) {
        return <ul {...props} className={`list-disc list-inside py-2 ${props.className}`}>{children}</ul>
      },
      ol({ children, ...props }) {
        return <ol {...props} className={`list-decimal list-inside py-1 ${props.className}`}>{children}</ol>
      },

      li({ children, ...props }) {
        return <li {...props} className={`list-item py-1 ${props.className}`}>{children}</li>
      },
      a({ children, ...props }) {
        return <a className="text-blue-500 hover:underline hover:text-blue-600" {...props}>{children}</a>
      },
      table({ children, ...props }) {
        return <table className="table-auto py-2" {...props}>{children}</table>
      },
      tr({ children, ...props }) {
      ...
      img({ children, ...props }) {
        return <img className="w-full py-2" {...props} />
      },
      p({ children, ...props }) {
        ...
        if (typeof children === 'string') {
          ...
          return <p className="py-3 whitespace-pre-wrap" {...props}>{children}</p>
        }
        return <p className="py-3 whitespace-pre-wrap" {...props}>{children}</p>
      }
