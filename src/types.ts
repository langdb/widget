  ...
  return input.map((m: ChatMessage) => {
    ...
    const content: ChatMessageContent[] = m.content_array
      .map((item) => {
        ...
        const dataContent = contentPart[1];
        if (type === "ImageUrl") {
          const resultImage: ChatMessageContentImage = {
            type: "image_url",
            image_url: {
          ...
          const resultAudio: ChatMessageWithAudio = {
            type: "audio",
            audio: {
          ...
          const resultText: ChatMessageText = {
            type: "text",
...
export async function createSubmitMessage(props: {
  ...
  if (!files || !files.length) {
    ...
    const contentImagesAudios: ChatMessageContent[] = await Promise.all(
      files
        ...
        .map(async (file: FileWithPreview) => {
          if (file.type.startsWith("audio/")) {
            const file_name = file.raw_file.name;
            // format is extracted from file name
            const format = file_name.split(".").pop();
            const submitData = file.base64?.split(",")[1];
            const resultAudio: ChatMessageWithAudio = {
              type: "input_audio",
              audio: {
    ...
    const textMessage: ChatMessageText = {
      type: "text",
      text: message,
    };
    const finalResult: ChatCompletionMessage = {
      role: "user",
...
export const createImageUrl = async (props: {
...
export async function createInnerMessage(props: {
  ...
  if (!files || !files.length) {
    ...
    const parts: MessageContentPart[] = await Promise.all(
      files
        // only image files are supported
        ...
        .map(async (file): Promise<MessageContentPart> => {
          ...
          return [MessageContentType.ImageUrl, imageUrl as string, null];
        }),
    );
    parts.push([MessageContentType.Text, message, null]);
    return parts;
  }
}
